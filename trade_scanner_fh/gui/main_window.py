...
from PyQt6.QtWidgets import (
    ...
    QDialogButtonBox, QDoubleSpinBox, QFileDialog, QHBoxLayout,
    QInputDialog, QLabel, QLineEdit, QMainWindow, QMessageBox,
    QPushButton, QSpinBox, QSplitter, QStatusBar, QTextEdit, QToolBar,
    QVBoxLayout, QWidget,
)

from .. import config, finnhub_client
from .. import hotkey as hotkey_mod
from ..data_engine import check_schema_version, rebuild_ticker
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Nasdaq weekly auto-refresh ref (replaces the Zacks one as the
        # only surviving piece of launch-time automation). Toggle in
        # Data → Earnings dates section. QSettings-persisted.
    ...
    def _build_ui(self):
        ...
        # Weekly Nasdaq auto-refresh toggle (Phase 5 of Finnhub augment).
        ...
        # stamps the timestamp so weekly counter resets either way.
        self.act_nasdaq_auto_refresh = QAction(
            "Auto-refresh Nasdaq calendar weekly", self,
        )
        ...
        self.act_nasdaq_auto_refresh.setToolTip(
            ...
            "bulk fill if it's been ≥ 7 days since the last run. Manual "
        ...
        # Nasdaq weekly check (toggle below).
    ...
    def _on_update_done(self, updated: int, errors: int):
        ...
        # Phase 5 of Finnhub augmentation: Zacks daily smart-refresh
        # auto-trigger removed — earnings refresh is fully manual now,
        # except for the weekly Nasdaq calendar sweep below.
        if self._nasdaq_auto_refresh_ref[0] and self._is_nasdaq_weekly_due():
            self._kick_off_nasdaq_auto_refresh()
    ...
    def _combined_finnhub_skip_set(self) -> set[str]:
        """Union of the universe blacklist + the Finnhub-specific
        blacklist. Computed at the start of each Finnhub fill so any
        tickers the user added to the main blacklist after the last
        Finnhub run get picked up automatically (idempotent — already-
        present entries don't grow the file)."""
        ...
        return set(self._finnhub_blacklist)
    ...
    def _zacks_skip_set(self) -> set[str]:
        """The combined skip set Zacks workers should honor: universal
        blacklist (everything skipped everywhere) ∪ zacks-only skip
        list (skipped by Zacks fills only). Centralized so all three
        ZacksFillWorker spawn sites stay consistent."""
    ...
    def _spot_fill_finnhub(self):
        """Menu: single-ticker Finnhub fetch via input dialog."""
        if self._finnhub_worker and self._finnhub_worker.isRunning():
            self.log_panel.write_line("Finnhub fill already running.")
            return
        if not self._check_finnhub_key_or_warn():
    ...
    def _load_greylist(self):
        """Load grey-listed tickers from file."""
    ...
    def _bulk_fill_earnings(self):
        ...
        def _on_done(f, e):
            # Phase 5: manual Nasdaq run resets the weekly counter so
            # the next auto-trigger waits a full week regardless of
    ...
    def _bulk_fill_zacks(self):
        ...
        # Pre-skip pass over universe.csv ETF/ADR flags.
    ...
    def _targeted_fill_zacks(self):
        ...
        if not has_zacks_cookies():
            ...
            return
        syms = self._get_universe_symbols()
        gaps = find_gap_tickers(syms, self._blacklist)
        if not gaps:
            QMessageBox.information(
                self, "No Gaps",
        ...
        if any(f.auto_fixable for f in findings):
            ...
            def _do_fix():
                if QMessageBox.question(
                    ...
                    return
                fixed, msgs = fix_integrity_issues(df, findings)
                save_earnings_history(fixed, sort=True)
                self.log_panel.write_line(
                    f"Integrity auto-fix applied "
                    f"({len(df) - len(fixed)} rows removed):"
                )
                for m in msgs:
    ...
    # ──────────────────────────────────────────────────────────────────
    # Nasdaq weekly auto-refresh (Phase 5 of Finnhub augmentation)
    # ──────────────────────────────────────────────────────────────────

    _NASDAQ_LAST_RUN_KEY = "menu/last_nasdaq_run_iso"

    def _on_nasdaq_auto_refresh_toggled(self, enabled: bool):
        """Toggle the weekly Nasdaq calendar auto-refresh."""
        ...
        state = "ENABLED" if enabled else "DISABLED"
        self.log_panel.write_line(
            f"Nasdaq weekly auto-refresh: {state} (saved)."
        )

    def _is_nasdaq_weekly_due(self) -> bool:
        """True iff the last recorded Nasdaq run is older than
        ``config.NASDAQ_WEEKLY_REFRESH_DAYS`` (or never ran)."""
        try:
        ...
        return gap.days >= config.NASDAQ_WEEKLY_REFRESH_DAYS
    ...
    def _stamp_nasdaq_run_now(self) -> None:
        ...
        action so they share the weekly counter."""
        try:
            self._qsettings().setValue(
    ...
    def _kick_off_nasdaq_auto_refresh(self) -> None:
        ...
        if not syms:
            return
        self.log_panel.write_line(
            "Nasdaq weekly auto-refresh: launching calendar sweep..."
        )
        ...
        self._earnings_worker.progress.connect(
            lambda d, t: self.status.showMessage(
                f"Nasdaq calendar (auto-weekly): {d}/{t}"
            )
        )
        self._earnings_worker.finished.connect(
    ...
    def _on_nasdaq_auto_refresh_done(self, filled: int, errors: int):
        self._stamp_nasdaq_run_now()
        self.status.showMessage(
            f"Nasdaq weekly auto-refresh done: {filled} filled, "
            f"{errors} errors"
        )
    ...
    def _run_scan(self):
        ...
        if sequenced:
            ...
            for days, chk in self.tf_checks.items():
                if chk.isChecked():
                    tf_start = qd_end.addDays(-days)
                    s = date(tf_start.year(), tf_start.month(), tf_start.day())
        ...
        self._worker = ScanWorker(filtered, params_list, sequenced=sequenced)
    ...
    def _export_results(self):
        ...
        try:
            if path.lower().endswith(".txt"):
                ...
                headers = [
                    RESULT_COLUMNS[c][0]
                    for c in range(min(cols, len(RESULT_COLUMNS)))
                ]
